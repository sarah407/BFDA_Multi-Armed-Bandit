...
n_indiv = 150
number_simulations = 5000 # total for all differences in mean


# Generating pseudo-random means for a set of differences in means
diff_mean = np.linspace(0, 0.95, 20)
means_all = np.array(
    np.meshgrid(
        diff_mean, np.random.uniform(0, 1,int(number_simulations/len(diff_mean)))
        )).T.reshape(-1,2)
...
BF_all = np.zeros((len(diff_mean), len(n_e_all), int(number_simulations/len(diff_mean))))
estimations_all = np.zeros((len(diff_mean), len(n_e_all), int(number_simulations/len(diff_mean))))

for i_diff, diff in enumerate(means_all):
    start_diff = time()
    print('----------------- \n', diff_mean[i_diff])
    for i_means, means in enumerate(diff):
        BF, estimations = main(1, means, n_e_all, likelihood, models, trials_per_game, n_indiv, prob_all, observed, obs_information, theta,prior_diff)
        
        BF_all[i_diff, :, i_means] = BF[:,0]
        estimations_all[i_diff, :, i_means] = estimations[:,0]
    end_diff = time()
...
with open('../data/data_mu-ss.pckl', 'wb') as f:
